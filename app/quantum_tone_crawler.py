    ...
    def analyze_text_tone(self, text: str) -> dict:
        ...
        try:
            response = openai.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "Analyze the tone of the following text: formality (formal/casual), complexity (low/medium/high), primary sentence type (declarative/interrogative/etc.). Respond with JSON only."},
                    {"role": "user", "content": text}
                ],
                max_tokens=200
            )
            neural_result = response.choices[0].message.content
            neural_data = json.loads(neural_result)

            formality = neural_data.get('formality', 'unknown')
            complexity = neural_data.get('complexity', 'unknown')
            primary_sentence_type = neural_data.get('primary_sentence_type', 'unknown')

        except Exception as e:
            self.logger.error(f"OpenAI API error: {str(e)}. Falling back to simplified analysis.")
            formality = 'unknown'
            complexity = 'unknown'
            primary_sentence_type = 'unknown'
        ...
        # Similarly for reasoning styles
